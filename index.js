...
function setDateDetails(date, isoString) {
  ...
  if (Date.parse(isoString) === NaN) return "Помилка: недійсний ISO рядок";
  else {
...
function nextNewYear() {
  ...
  let newYear = new Date(++thisYear, 0, 1);
...
function getDaysInMonth(date) {
  ...
  let nextDay = new Date(year, mon + 1, 1);

  nextDay.setDate(nextDay.getDate() - 1);

  return {
    inputDate,
    daysInMonth: nextDay.getDate(),
  };
}
...
function getDaysDifference(startDate, endDate) {
  ...
  let daysDifference = (endDate - startDate) / (1000 * 60 * 60 * 24);
